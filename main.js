...
ipcMain.handle('excel:saveRraaCriterios', async (_event, payload) => {
  try {
    ...
    const raPesos = Array.isArray(payload.raPesos) ? payload.raPesos : [];

    await saveRraaCriteriosToFile(selectedExcelPath, rraa, criterios, ponderacionesUnidad, raPesos);
    return loadRraaCriteriosFromSelectedFile();
  } catch (error) {
...
function loadRraaCriteriosFromSelectedFile() {
  ...
  return {
    ...
    criterios,
    ponderacionesUnidad,
    raPesos: extractRaPesos(pesosRows, rraa, criterios)
  };
}
...
function extractRraaCriteriosFromWorkbook(workbook) {
  ...
  if (rraaStart === -1) return { rraa: [], criterios: [], ponderacionesUnidad: [], raPesos: [] };
  ...
  return { rraa, criterios, ponderacionesUnidad, raPesos: extractRaPesos(pesosRows, rraa, criterios) };
}
...
async function saveRraaCriteriosToFile(filePath, rraa, criterios, ponderacionesUnidad, raPesos = []) {
  ...
  const normalizedRaPesos = normalizeRaPesosForSave(raPesos, normalizedCriterios);
  ...
  await editWorkbookSheetsXml(filePath, {
    ...
    PESOS: (sheetXml) => {
      ...
      normalizedRaPesos.forEach((peso) => {
        xml = setXmlCell(xml, 24, peso.colIdx, peso.instituto, 'number');
        xml = setXmlCell(xml, 25, peso.colIdx, peso.empresa, 'number');
      });
...
function raStartColumn(raNumero) {
  const ra = Number(raNumero) || 1;
  return 2 + Math.max(0, ra - 1) * 40;
}

function firstCriterionColumnForRa(criterios, raNumero) {
  const cols = criterios
    .filter((criterio) => Number(criterio.raNumero) === Number(raNumero))
    .map((criterio) => Number(criterio.colIdx))
    .filter((colIdx) => Number.isInteger(colIdx) && colIdx >= 0)
    .sort((a, b) => a - b);
  return cols.length ? cols[0] : raStartColumn(raNumero);
}

function firstNumberInColumns(rows, rowIdx, cols) {
  const row = rows[rowIdx] || [];
  for (const colIdx of cols) {
    const value = parseDecimal(row[colIdx]);
...
function extractRaPesos(pesosRows, rraa, criterios) {
  const raNumeros = [...new Set([
    ...rraa.map((item) => Number(item.numero)),
    ...criterios.map((item) => Number(item.raNumero))
  ])].filter(Boolean).sort((a, b) => a - b);

  return raNumeros.map((raNumero) => {
    const cols = criterios
      .filter((criterio) => Number(criterio.raNumero) === Number(raNumero))
      .map((criterio) => Number(criterio.colIdx))
      .filter((colIdx) => Number.isInteger(colIdx) && colIdx >= 0)
      .sort((a, b) => a - b);
    const start = cols.length ? cols[0] : raStartColumn(raNumero);
    const end = (cols.length ? cols[cols.length - 1] : start) + 2;
    const scanCols = Array.from({ length: end - start + 1 }, (_, idx) => start + idx);
    const inst = firstNumberInColumns(pesosRows, 24, scanCols);
    const emp = firstNumberInColumns(pesosRows, 25, scanCols);
    const rraaItem = rraa.find((item) => Number(item.numero) === Number(raNumero));
    const colIdx = (inst && inst.colIdx) || (emp && emp.colIdx) || firstCriterionColumnForRa(criterios, raNumero);
    const instituto = inst ? inst.value : 0;
    const empresa = emp ? emp.value : 0;
    ...
    return {
      ...
      instituto,
      empresa,
      total: Number((instituto + empresa).toFixed(6)),
      colIdx
    };
  });
}

function normalizeRaPesosForSave(raPesos, criterios) {
  return raPesos
    .map((item) => {
      ...
      return {
        raNumero,
        instituto: parseDecimal(item.instituto),
        empresa: parseDecimal(item.empresa),
        colIdx: item.colIdx !== null && item.colIdx !== undefined && Number.isInteger(Number(item.colIdx))
          ? Number(item.colIdx)
          : firstCriterionColumnForRa(criterios, raNumero)
      };
    })
    .filter(Boolean);
}
...
async function commandSaveRraaCriterios(payload) {
  ...
  const raPesos = Array.isArray(payload && payload.raPesos) ? payload.raPesos : [];
